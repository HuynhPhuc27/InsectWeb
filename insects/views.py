...
from .models import InsectsImage, Species, InsectsBbox, Genus, RequestDesc, Document, Class, Family, Order, Phylum, \
    PasswordResetOTP, RequestImage, InsectsCrawler, VerificationLog, ClassificationHistory, ImageClassification, ImageVote, CaptionHistory
...
def append_insect(request):
    ...
    request_insect = Request.objects.filter(user=request.user).order_by('-created_at')
...
class PatchWiseCrossAttention(nn.Module):
    ...
    def forward(self, text_features, patch_features):
        ...
        # Project patches to key and value
...
#Classification
def visualize_attention(image, attention_weights, image_path, text, pred_class, confidence, output_dir, patch_size=16):
    image_np = np.array(image)
    height, width = image_np.shape[:2]

    attn = attention_weights.mean(dim=1).squeeze(1)
    attn = attn[0].cpu().numpy()
    attn_patches = attn.shape[0]

    grid_size = int(np.sqrt(attn_patches))
    if grid_size * grid_size != attn_patches:
        raise ValueError("Số lượng patch không phải là số chính phương.")
        return None
    ...
    overlay = cv2.addWeighted(image_np, 0.6, heatmap, 0.4, 0.0)
    os.makedirs(output_dir, exist_ok=True)
    output_path = os.path.join(output_dir, f"visualize_{os.path.basename(image_path)}")
...
def load_models(weights_path):
    print("🔹 Loading ViT, BERT, and classifier...")
    ...
    # Class mapping
    idx_to_class_id = {idx: class_id for class_id, idx in checkpoint['class_id_to_idx'].items()}
...
def get_model(weights_path):
    ...
    if MODEL_CACHE is None:
        MODEL_CACHE = load_models(weights_path)
    return MODEL_CACHE

def visualize_user_input(image_path, text, weights_path, output_dir='media/attention_visualizations'):
    models = get_model(weights_path)
    vit_model = models["vit_model"]
    vit_processor = models["vit_processor"]
    bert_model = models["bert_model"]
    bert_tokenizer = models["bert_tokenizer"]
    classifier = models["classifier"]
    idx_to_class_id = models["idx_to_class_id"]
    ...
    # Process text
    text_inputs = bert_tokenizer(text, return_tensors="pt", padding=True, truncation=True, max_length=128)
    text_inputs = {k: v.to(device) for k, v in text_inputs.items()}

    # Get embeddings and classification
    with torch.no_grad():
        ...
        logits, attention_weights = classifier(text_embeddings, patch_embeddings)
        probs = torch.softmax(logits, dim=-1).cpu().numpy()[0]

        # Lấy class có xác suất cao nhất
        pred_idx = int(np.argmax(probs))
        pred_class = str(idx_to_class_id[pred_idx])
        confidence = float(probs[pred_idx])


    # Visualize attention
    vis_path = visualize_attention(
        image=image,
        ...
        output_dir=os.path.join(settings.MEDIA_ROOT, 'attention_visualizations'),
        patch_size=16
    )
...
def classify(request):
    if request.method == 'POST':
        ...
        if not uploaded_file:
            return JsonResponse({'error': 'Please load new image'}, status=400)

        if not description:
            return JsonResponse({'error': 'Please write image caption'}, status=400)
        ...
        try:
            #weights_path = os.path.join(settings.MEDIA_ROOT, 'model', 'weights_images_florence_wiki.pth')
            weights_path = r"D:/KL_DTH225725/Train Model/Loss/weights_images_florence_wiki.pth"
            result = visualize_user_input(image_path, description, weights_path)

            return JsonResponse({
                "original": request.build_absolute_uri('/media/uploads/' + new_filename),
                "visualized": request.build_absolute_uri(settings.MEDIA_URL + result["visualize_path"]),
                "pred_class": result["pred_class"],
                "confidence": float(result["confidence"]),
            })

        except Exception as e:
            print(f"Lỗi trong classify view: {str(e)}")
            return render(request, 'classify.html', {'error': f"Lỗi khi phân loại: {str(e)}"})
...
def save_history(request):
    if request.method == 'POST':
        try:
            ...
            def save_images(img_str, folder):
                if not img_str:
                    return None
                ...
                if img_str.startswith("data:image"):
                    ...
                    ext = format.split('/')[-1]
                    filename = f"{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}_{uuid.uuid4().hex[:6]}.{ext}"
                ...
                elif '/media/' in img_str:
                    relative_path = img_str.split('/media/')[-1]
                    return relative_path

                return None
            
            original_path = save_images(data.get('original_image'), 'upload')
            visualize_path = save_images(data.get('visualized_image'), 'attention_visualizations')
            print("Original: ", original_path)
            print("Visualize:", visualize_path)
            record = ClassificationHistory.objects.create(
                user=user,
                original_image=original_path,
                visualize_image=visualize_path,
...
def delete_temp_images(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body.decode('utf-8'))
            original_url = data.get('original')
            visualized_url = data.get('visualized')
            deleted_files=[]
            ...
            delete_file_from_url(original_url)
            delete_file_from_url(visualized_url)
...
def classification_history(request):
    ...
    search_query = request.GET.get('search_class','')
    histories = ClassificationHistory.objects.all().order_by('-created_at')
...
def image_classification(request):
    history_queryset = CaptionHistory.objects.select_related('user').order_by('-changed_at')
    images = ImageClassification.objects.prefetch_related(
        Prefetch('edit_histories', queryset=history_queryset, to_attr='last_edit')
    ).order_by('id')
    
    voted_image_ids = []
    if request.user.is_authenticated:
        voted_image_ids = ImageVote.objects.filter(user=request.user).values_list('image_id', flat=True)
    ...
    context = {
        "images": page_obj,
        "page_obj": page_obj,
        "voted_image_ids": voted_image_ids,
        "MEDIA_URL": settings.MEDIA_URL,
    }
    return render(request, "data_classification.html", context)


def edit_caption(request, img_id):
    image_item = get_object_or_404(ImageClassification, id=img_id)
    if request.method == "POST":
        old_caption = image_item.caption
        ...
        if old_caption != new_caption:
            ...
            image_item.caption = new_caption
            image_item.save()
            
        return redirect('/data_classification/')

@login_required 
def vote_image(request, img_id):
    image_item = get_object_or_404(ImageClassification, id=img_id)
    
    vote_exists = ImageVote.objects.filter(user=request.user, image=image_item).exists()
    
    if not vote_exists:
        ImageVote.objects.create(user=request.user, image=image_item)
        image_item.votes += 1
        image_item.save()
        return JsonResponse({'success': True, 'new_votes': image_item.votes, 'status': 'voted'})
    else:
        ImageVote.objects.filter(user=request.user, image=image_item).delete()
        image_item.votes -= 1
        image_item.save()
        return JsonResponse({'success': True, 'new_votes': image_item.votes, 'status': 'unvoted'})

def dataset_statistics(request):
    total_images = ImageClassification.objects.count()
    total_votes = ImageClassification.objects.aggregate(total=Sum("votes"))["total"] or 0
    total_classes = ImageClassification.objects.values('species').distinct().count()

    class_distribution = ImageClassification.objects.values('species__name').annotate(count=Count('id')).order_by('-count')
    life_stage = ImageClassification.objects.values('life_stage').annotate(count=Count('id')).order_by('-count')

    class_temp = ImageClassification.objects.values('species__name').annotate(count=Count('id'))
    stage_map = {
        '0': "Egg",
        '1': "Larva",
        '2': "Pupa",
    ...
    for i in class_distribution:
        class_labels.append(i['species__name'])
        class_counts.append(i['count'])
    
    stage_labels = []
    stage_counts = []
    for i in life_stage:
        stage_labels.append(stage_map.get(str(i['life_stage']), 'Unknown'))
        stage_counts.append(i['count'])
    ...
    return render(request, 'dataset_statistics.html', context)
...
def is_expert(user):
    return user.is_authenticated and (user.groups.filter(name='CVs').exists() or user.is_superuser)
...
def edit_caption(request, img_id):
    ...
    return render(request, "edit_caption.html", {
        "image": image_item,
        "MEDIA_URL": settings.MEDIA_URL
    })
...
def verify_new_image_request(request, request_img_id):
    ...
    img_url = request.build_absolute_uri(settings.MEDIA_URL + str(request_item.url))

    # Đọc dữ liệu bbox từ file txt
    img_path = os.path.join(settings.MEDIA_ROOT, request_item.url)
    # Gọi hàm vẽ bounding box
    bbox_data = readfile_boundingbox(img_path)
    print("bbox: ", img_path)
    ...
    if request.method == "POST":
        ...
        # exiting_vote = VerificationHistory.objects.filter(
        #     request_img_id = request_item,
        #     user_id = request.user
        # )
        # if exiting_vote:
        #     messages.error(request, "Bạn đã vote hình ảnh này rồi!")
        ...
        if action == "accept":
            if request_item.verification_count >= 3:
                ...
                try:
                    species_infor = Species.objects.get(insects_id=insects_id)  # Lấy đối tượng Species từ ID
                except Species.DoesNotExist:
                    messages.error(request, "Loài côn trùng không tồn tại!")
                    return redirect('cv_verify_new_image')

                # request_item.verification_count += 1  # Tăng số chuyên gia đã xác minh
                # request_item.insects_id = species_infor
                # request_item.desc = description
                if request_item.verification_count >= 3:
                    request_item.status = "verified"  # Đủ 3 chuyên gia thì chấp nhận
        ...
        # return redirect('cv_verify_new_image')  # Điều hướng về trang danh sách yêu cầu

    context = {
        'request_item': request_item,
        'species_list': species_list,
        'specie': specie,
        'bbox_img_url': img_url,
        'bbox_data': bbox_data,
        'has_verified': has_verified,
...
def readfile_boundingbox(img_path):
    if not os.path.exists(img_path):
        return None  # Ảnh không tồn tại

    bbox_txt = os.path.splitext(img_path)[0] + ".txt"
    if not os.path.exists(bbox_txt):
        return None  # Không có file bbox

    bbox_data = []

    # Đọc dữ liệu bbox từ file
    with open(bbox_txt, 'r') as file:
        for line in file:
            bbox_data.append(line.strip())  # Lưu từng dòng vào danh sách
...
def save_bbox_verify(request, request_img_id):
    ...
    try:
        ...
        if request_item.verification_count >= 3:
            request_item.status = 'verified'
            messages.success(request, "Hình ảnh đã được xét duyệt thành công!")
        else:
            messages.info(request,
                          f"Hiện có {request_item.verification_count}/3 chuyên gia chấp nhận. Cần thêm {3 - request_item.verification_count} chuyên gia nữa!")
        request_item.save()

        # Lưu bbox vào file .txt
        img_path = os.path.join(settings.MEDIA_ROOT, request_item.url.lstrip('/'))
        bbox_txt = os.path.splitext(img_path)[0] + '.txt'
        if new_bbox_lines:
            with open(bbox_txt, 'w') as file:
                for line in new_bbox_lines:
                    file.write(line + '\n')
        else:
        ...
        return JsonResponse({'status': 'success', 'message': 'Lưu bbox và xét duyệt thành công'})
    ...
    if request.method == "POST":
        ...
        if action == "accept":
            ...
            new_image = InsectsImage.objects.create(
                img_id=new_filename[:-4],  # Lưu mã ID không có phần mở rộng
                insects_id=request_item.insects_id.pk,  # Gán đối tượng Species
                url=f"images/{new_filename}",  # Lưu đường dẫn mới
            ...
            bbox_file_old = img_path.replace(".jpg", ".txt")
            bbox_file_new = new_img_path.replace(".jpg", ".txt")
            if os.path.exists(bbox_file_old):
                shutil.move(bbox_file_old, bbox_file_new)
            ...
            request_item.status = "accepted"
            request_item.url = f"images/{new_filename}"  # Cập nhật đường dẫn mới
            request_item.save()

            messages.success(request, "Mô tả đã được chấp nhận và lưu vào hệ thống!")
        ...
        return redirect('admin_verify_new_image')  # Điều hướng về trang danh sách yêu cầu
