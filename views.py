...
def classify(request):
    if request.method == 'POST':
        ...
        if not uploaded_file:
            return JsonResponse({'error': 'Vui lòng tải lên một hình ảnh.'}, status=400)

        if not description:
            return JsonResponse({'error': 'Vui lòng nhập mô tả văn bản.'}, status=400)
...
def image_classification(request):
    images = ImageClassification.objects.all().order_by('id')
    history_queryset = CaptionHistory.objects.select_related('user').order_by('-changed_at')
    images_list = ImageClassification.objects.prefetch_related(
