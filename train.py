...
# Step 1: Custom Dataset for loading images and texts
class MultimodalDataset(Dataset):
    def __init__(self, df, vit_processor, bert_tokenizer, root_images, class_id_to_idx=None, num_classes=None):
        ...
        if class_id_to_idx is None:  # train
    ...
    def __getitem__(self, idx):
        ...
        # Process image for ViT (no cropping, use full image)
        image_inputs = self.vit_processor(images=image, return_tensors="pt")
        # Process text for BERT
        text_input = text
...
def make_collate_fn(bert_tokenizer):
    def collate_fn(batch):
        ...
        # Encode text đồng bộ bằng tokenizer (padding trong batch)
...
# Step 5: Training function with evaluation
def train_model(root_images, train_csv='train.csv', val_csv='val.csv', test_csv='test.csv', output_pth='weights.pth', batch_size=16, num_epochs=10):
    ...
    # Load datasets
    train_df = pd.read_csv(os.path.join(root_images, train_csv))
    val_df = pd.read_csv(os.path.join(root_images, val_csv))
    test_df = pd.read_csv(os.path.join(root_images, test_csv))
    ...
    for epoch in range(num_epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            torch.save({
                ...
                'class_id_to_idx': train_dataset.class_id_to_idx
            }, "best_weights_images_llava_wiki.pth")
    ...
    # Save weights and additional info
    torch.save({
        'model_state_dict': classifier.state_dict(),
        'class_ids': train_dataset.class_ids,
        'class_id_to_idx': train_dataset.class_id_to_idx,
        'embeddings': [],  # To store final embeddings
        'attention_weights': [],  # To store attention weights for visualization
        'image_paths': [],
        'species': []
    }, output_pth)

    # Generate embeddings and attention weights for all samples (using train dataset)
    classifier.eval()
    all_embeddings = []
    ...
    with torch.no_grad():
        for batch in tqdm(test_dataloader, desc="Generating embeddings"):
            ...
            # Get attended features and attention weights
    ...
    # Update .pth file with embeddings and attention weights
    torch.save({
        'model_state_dict': classifier.state_dict(),
        'class_ids': train_dataset.class_ids,
        'class_id_to_idx': train_dataset.class_id_to_idx,
...
# Train and save weights with evaluation
train_model(root_images, train_csv=train_csv, val_csv=val_csv, test_csv=test_csv, output_pth='...', batch_size=16, num_epochs=15)
